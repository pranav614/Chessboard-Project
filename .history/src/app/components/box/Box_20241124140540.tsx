...
const Box: React.FC<BoxProps> = ({ chessObj, chessBoard }) => {
  ...
  const handleRookPossibleMoves = useCallback(() => {
    const updatedMatrix = chessBoard.map((row) => {
      row.map((column) => {
        return { ...column, possibleMoveColor: "" };
      });
      return {
        ...row,
      };
    });
    console.log(chessObj.i, chessObj.j);
    const directions = [
      { dx: 0, dy: 1 }, //right move
      { dx: 0, dy: -1 }, //left move
      { dx: 1, dy: 0 }, //bottom move
      { dx: -1, dy: 0 }, //top move
    ];
    directions.map(({ dx, dy }) => {
      ...
      while (true) {
        ...
        if (
          ...
          updatedMatrix[x][y].possibleMoveColor = "#B9CA43";
        } else {
          break;
        }
      }
    ...
    console.log(updatedMatrix);
    dispatch(addChessBoard)
  },[chessBoard, chessObj.i, chessObj.j]);
