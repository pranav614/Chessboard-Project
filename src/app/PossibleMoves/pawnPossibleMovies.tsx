...
export const pawnPossibleMoves = ({ chessObj, chessBoard, PAWN_POSSIBLE_MOVES }: IPossibleMoves): chessPiece[][] => {
    ...
    PAWN_POSSIBLE_MOVES.forEach(({ dx, dy }, index) => {
        ...
        if ((x >= 0 && x <= 7) && (y >= 0 && y <= 7)) {
            ...
            if (piece.chessPiece === "" && piece.possibleMoveColor === "" && (index === 0 || index === 3)) {
                piece.possibleMoveColor = "#B9CA43";

            } else if (piece.chessPiece !== "" && piece.color !== chessObj?.color && (index === 1 || index === 2)) {
